...
class Calibrate_Camera:
    ...
    def Undistor(self,img,Camera_Matrix, Dist_Coeffs):
        h,  w = img.shape[:2]
        NewCam_Matrix, ROI = cv2.getOptimalNewCameraMatrix(Camera_Matrix,Dist_Coeffs,(w,h),1,(w,h))
        mapx,mapy = cv2.initUndistortRectifyMap(Camera_Matrix,Dist_Coeffs,None,NewCam_Matrix,(w,h),5)
    ...
    def Camera_Calibrate(self,C_Parameters = "Camera_Parameter.yaml",patternsize = [9,6]):
        ...
        images=sorted(glob.glob("ChessBoard/*.jpg"), key=os.path.basename)
        print(images)
        i=0
        for fname in images:
            print(fname.split("/")[1])
            ...
            if ret == True:
                ...
                cv2.imwrite('Corners/Corners_'+fname.split("/")[1],img)
                #cv2.waitKey(50)
                i=i+1

        cv2.destroyAllWindows()
        RMS, Camera_Matrix, Dist_Coeffs, Rotation_V, Translation_V = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1],None,None)

        mean_error = 0
        tot_error = 0
        for i in range(len(objpoints)):
            imgpoints2, _ = cv2.projectPoints(objpoints[i], Rotation_V[i], Translation_V[i], Camera_Matrix, Dist_Coeffs)
        ...
        for fname in images:
            ...
            cv2.imwrite('Undistort/Undistort_'+fname.split("/")[1],Dist)
            #cv2.waitKey(50)
            i=i+1
